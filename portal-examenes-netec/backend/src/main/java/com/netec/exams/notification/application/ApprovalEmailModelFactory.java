...
public class ApprovalEmailModelFactory {
  ...
  public ApprovalEmailModel create(ExamRequest request, String approverName, Instant submittedAt){
    ...
    var commercial=new ApprovalEmailModel.CommercialSummary(nvl(request.scheduledCourseCode()),nvl(request.courseType()),nvl(request.segment()),nvl(request.costCenter()),nvl(request.companyNameSnapshot()),nvl(request.billingReference()),nvl(request.salesAdvisorNameSnapshot()));
