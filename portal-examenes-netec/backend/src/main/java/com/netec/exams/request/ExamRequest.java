...
public class ExamRequest {
  ...
  public void apply(DraftCommand c, Instant now) { if(status!=ExamRequestStatus.BORRADOR) throw new InvalidStateException(); scheduledCourseCode=trim(c.scheduledCourseCode()); siteCodeSnapshot=trim(c.siteCode()); companyNameSnapshot=company(c.companyName()); billingReference=trim(c.billingReference()); observations=trim(c.observations()); updatedAt=now; }
