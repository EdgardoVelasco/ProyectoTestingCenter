...
class ExamRequestController {
  ...
  @PostMapping("/{id}/submit") ResponseEntity<ApprovalNotificationService.NotificationResult> submit(@PathVariable UUID id,@RequestHeader(value="Idempotency-Key",required=false) String idempotencyKey,Authentication auth){var x=service.entity(id,requester(auth));return ResponseEntity.ok(notifications.submit(x,idempotencyKey));}
