...
@Service public class ApprovalNotificationService {
 ...
 @Transactional public NotificationResult submit(ExamRequest request){return submit(request, null);}
 @Transactional public NotificationResult submit(ExamRequest request, String requestedIdempotencyKey){
  ...
  var key=requestedIdempotencyKey==null||requestedIdempotencyKey.isBlank()
      ? "approval-exam-request:"+request.id()+":v"+request.version()
